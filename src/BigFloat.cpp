...
BigFloat BigFloat::sqrt(const BigFloat &x) {
  ...
  BigFloat guess = BigFloat(1, work_prec).mul_pow2(total_exp / 2);
  // 预计算 0.5
...
BigFloat BigFloat::sin(const BigFloat &x) {
  ...
  for (int k = 1;; k++) {
    term *= reduced_sq;
    // 除以 2k * (2k+1)
    auto divisor = static_cast<uint32_t>(2 * k * (2 * k + 1));
...
std::string BigFloat::to_decimal_string(int decimal_digits,
  ...
  int64_t total_bits = exponent_ + mantissa_.bit_length();
  int64_t log_val = total_bits * 30103LL; // log10(2) * 100000 ≈ 30103
  int64_t e10 =
      (log_val >= 0) ? (log_val / 100000LL) : ((log_val - 99999LL) / 100000LL);
  ...
  // --- 步骤 2: 确定缩放因子 ---
  // --- 步骤 2: 确定缩放因子（引入额外保护位） ---
  // 为了防止 e10 估算误差导致最后一位被截断，我们多提取 5 位十进制数字
  int extra_guard = 5;
  int shift_10;
  if (scientific_notation) {
    shift_10 = decimal_digits - static_cast<int>(e10) + extra_guard;
  } else {
    shift_10 = decimal_digits + extra_guard;
  }
  ...
  if (net_exp > 0) {
    ...
    int shift_right = static_cast<int>(-net_exp);
    BigInt half = BigInt(1) << (shift_right - 1);
    scaled = (scaled + half) >> shift_right;
  }
  ...
  // --- 步骤 4: 格式化与十进制舍入 ---
  if (scientific_notation) {
    int drop_count = static_cast<int>(digits.size()) - (decimal_digits + 1);
    int exp_offset = 0;

    if (drop_count > 0) {
      char round_char = digits[digits.size() - drop_count];
      digits.erase(digits.size() - drop_count);
      // 进位处理
      if (round_char >= '5') {
        ...
        if (i >= 0) {
          ...
          digits.pop_back(); // 保持长度恒定
          exp_offset = 1;
        }
      }
    } else if (drop_count < 0) {
      digits.append(-drop_count, '0');
      drop_count = 0; // 为了正确计算指数，把额外补充的视为没有drop
    }

    // 推导正确的科学计数法指数
    int actual_e10 = static_cast<int>(digits.size()) + drop_count - 1 -
                     shift_10 + exp_offset;
    ...
    result += (actual_e10 >= 0) ? "e+" : "e";
    result += std::to_string(actual_e10);
  } else {
    // 定点数格式
    // 补齐前导零确保小数点有位置安放（至少需要 shift_10 + 1 位数字）
    int orig_len = static_cast<int>(digits.size());
    if (orig_len <= shift_10) {
      digits.insert(0, shift_10 - orig_len + 1, '0');
    }

    int drop_count = extra_guard;
    char round_char = digits[digits.size() - drop_count];
    digits.erase(digits.size() - drop_count);
    // 进位处理
    if (round_char >= '5') {
