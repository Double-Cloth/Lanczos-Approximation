...
static BigFloat arctan_recip(int x, int prec_bits) {
    ...
    for (int k = 0;; k++) {
        BigFloat denom(2 * k + 1, prec_bits); // 分母: 2k+1
        BigFloat term = power / denom; // 当前项: power / (2k+1)
...
BigFloat BigFloat::exp(const BigFloat &x) {
    ...
    BigFloat term(1, work_prec); // term_k = reduced^k / k!
    for (int k = 1;; k++) {
        term *= reduced; // term *= reduced
        term /= BigFloat(k, work_prec); // term /= k
        sum += term;

        // 收敛检查: 当前项相对于累积和足够小时停止
        if (term.is_zero())
            break;
        int term_bl =
