...
int main(int argc, char *argv[]) {
    ...
    if (mode_flag == "eval") {
        ...
        // 确定显示精度
        int display_digits = (custom_display_digits > 0)
                                 ? custom_display_digits
                                 : std::min(digits + 5, 40);
    ...
    // --- 保存系数文件 ---
    {
        ...
        fout << "#         S(z) = p[0] + sum_{k=1}^{n-1} p[k] / (z + k)"
                << std::endl;
    ...
    vout << "#" << std::endl;
    vout << "# z, computed, expected (first 30 digits), absolute_error, "
            "matching_digits, status"
            << std::endl;
    ...
    for (int idx = 0; idx < total_tests; idx++) {
        ...
        int expected_prec_bits =
                static_cast<int>(entry.expected_str.length() * 3.3219281) + 64;
        ...
        std::string fixed_expected = fix_expected_value(
            gamma_val.to_decimal_string(10, false), entry.expected_str);
        ...
        if (expected_val.is_zero()) {
            ...
            if (c_e_pos != std::string::npos && e_e_pos != std::string::npos) {
                // 比较指数部分（整数比较，避免格式差异如 e+0 vs e+00）
        ...
        std::cout << "  Gamma(" << entry.z_str << ") = " << computed_str
                << "  [Error: " << abs_err_str << ", " << matching << " digits, "
                << status << "]" << std::endl;

        vout << entry.z_str << ", " << computed_str << ", " << expected_short
                << "..."
                << ", " << abs_err_str << ", " << matching << ", " << status
                << std::endl;
    }
    ...
    vout << "# Passed (>= 8 digits): " << pass_count << "/" << total_tests
            << std::endl;
    ...
    std::cout << "  Passed (>= 8 digits): " << pass_count << "/" << total_tests
            << std::endl;
