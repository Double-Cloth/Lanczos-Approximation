...
public:
    ...
    /** @brief 获取当前精度（二进制位数） */
    [[nodiscard]] int precision() const { return precision_bits_; }
     ...
     *          如果新精度大于当前尾数位长，会左移扩展
     */
    void set_precision(const int bits) {
     ...
     */
    [[nodiscard]] BigFloat floor() const;

    /**
     * @brief 判断该浮点数所代表的值是否为奇整数
     * @return 如果值是精确的整数且为奇数，则返回 true；否则 false
     */
    [[nodiscard]] bool is_odd_integer() const;
     ...
     * @param n 指数偏移量（可为负数）
     */
    [[nodiscard]] BigFloat mul_pow2(int n) const;
     ...
     */
    [[nodiscard]] BigFloat div_u32(uint32_t val) const;
    ...
    /** @brief 只读访问尾数 */
    [[nodiscard]] const BigInt &mantissa() const { return mantissa_; }
